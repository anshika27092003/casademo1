...
def resolve_ck_amk_worksheet(spreadsheet):
    """Resolve CK AMK settlement tab strictly by gid and AMK title guard."""
    try:
        ws = spreadsheet.get_worksheet_by_id(CK_AMK_SETTLEMENT_GID)
        title_u = (ws.title or "").upper()
        # Safety guard: never allow CK sync into non-AMK tab.
        if "AMK" not in title_u:
            logger.error(
                "CK target gid resolved to non-AMK worksheet | gid=%s title=%s",
